...
def get_halo_number_density():
    ...
    try:
        ...
        print(f"暗晕数量 (1e11-3.16e11范围): {count_11_1}")
        print(f"暗晕数密度 (1e11-3.16e11范围): {density_11_1:.6e} (h/Mpc)^3")
        print(f"暗晕数量 (3.16e11-1e12范围): {count_11_2}")
        print(f"暗晕数密度 (3.16e11-1e12范围): {density_11_2:.6e} (h/Mpc)^3")
        ...
        # 使用估计值
        estimated_densities = {
            'halo_11_1': 1e-3,  # 1e11-3.16e11范围
            'halo_11_2': 5e-4   # 3.16e11-1e12范围（数量较少）
        }
...
def plot_power_spectra_shot_noise_corrected(k, Pm, Pg, Ph, Ph_extra, b1_k, b1_mean, 
    ...
    if Ph is not None:
        Ph_corrected = subtract_shot_noise(k, Ph, halo_densities['halo_11_1'])  # 使用1e11范围的密度
    else:
        Ph_corrected = None
    ...
    if Ph is not None and Ph_corrected is not None:
        ax1.loglog(k, Ph, 'b--', lw=1.5, alpha=0.7, label='Halo P(k) 1e11-3.16e11 (原始)')
        ax1.loglog(k, Ph_corrected, 'b-', lw=2, label='Halo P(k) 1e11-3.16e11 (减去shot noise)')
    
    if Ph_extra is not None and Ph_extra_corrected is not None:
        ax1.loglog(k, Ph_extra, 'g--', lw=1.5, alpha=0.7, label='Halo P(k) 3.16e11-1e12 (原始)')
        ax1.loglog(k, Ph_extra_corrected, 'g-', lw=2, label='Halo P(k) 3.16e11-1e12 (减去shot noise)')
...
def plot_power_spectra_shot_noise_corrected_dual(k, Pm, Pg, Ph1, Ph2, b1_k, b1_mean, 
    ...
    if Ph1 is not None and Ph1_corrected is not None:
        ax1.loglog(k, Ph1, 'b--', lw=1.5, alpha=0.7, label='Halo P(k) 1e11-3.16e11 (原始)')
        ax1.loglog(k, Ph1_corrected, 'b-', lw=2, label='Halo P(k) 1e11-3.16e11 (减去shot noise)')
    
    if Ph2 is not None and Ph2_corrected is not None:
        ax1.loglog(k, Ph2, 'g--', lw=1.5, alpha=0.7, label='Halo P(k) 3.16e11-1e12 (原始)')
        ax1.loglog(k, Ph2_corrected, 'g-', lw=2, label='Halo P(k) 3.16e11-1e12 (减去shot noise)')
...
def plot_all_power_spectra_dual(k_m, Pm, Ph1, Ph2, galaxy_results, output_file):
    ...
    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"双暗晕功率谱比较图已保存到: {output_file}")
    plt.close()
